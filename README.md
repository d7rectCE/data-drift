...
| правило | `"bonferroni"`; `"bh_window"`, если модели часто дрейфуют одновременно | Бонферрони держит уровень при любой корреляции моделей (эксп. 13), BH быстрее при массовых дрейфах (эксп. 4); при коррелированных моделях BH — только с `split_common=True` (эксп. 18) |
