...
ухудшение задаются явно. Подробности и все 18 экспериментов — в
...
| правило | `"bonferroni"`; `"bh_window"`, если модели часто дрейфуют одновременно | Бонферрони держит уровень при любой корреляции моделей (эксп. 13), BH быстрее при массовых дрейфах (эксп. 4); при коррелированных моделях BH — только на остатках `split_common` (эксп. 18) |
...
experiments/       18 экспериментов (exp1…exp18)
...
tests/             73 теста: совпадение с river, бутстреп, калибровка, процедуры, потоковый режим
