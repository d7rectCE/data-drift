...
[English](experiments.md) · **Русский**
...
3. **Узкое место калибровки — глубокий хвост.** При экспоненциальном хвосте ошибка
   ...
   описанным Wu & Apley; здесь нужна поправка на изменчивость оценки нуля. *(Позже сделано:
   `sieve_pu`, эксп. 6; GPD-хвост и B = 2000, эксп. 10.)*
...
5. **Двусторонняя гипотеза пока склоняется ко второму исходу.** В однородном
   ...
   порога на поток. Где она может окупиться — открытый вопрос для этапа 2. *(Позже уточнено:
   BH в окне выигрывает при кластерных дрейфах, эксп. 4; онлайн-FDR проигрывает везде, эксп. 12, 21;
   сама поправка на число моделей окупается во всех сценариях бенчмарка, эксп. 21.)*
...
районами 77%, но корреляция ошибок моделей может быть ниже, и это нужно измерить (сделано в
эксперименте 20).
...
кластерного дрейфа из эксперимента 4: внешне они выглядят одинаково (ответ — эксперимент 18).
...
`sieve_pu` оставлен как метод по умолчанию для реальных данных (эксперимент 7), а теперь это
метод по умолчанию в библиотеке.
...
INSECTS и Electricity (только естественные изменения, без внедрённого дрейфа) целиком, Airlines и
Covertype — первые 100 тыс. строк; строки в исходном порядке, по 50 моделей, 2 парка. Истина —
...
- **На Airlines метод не помогает.** 1000 строк там — около полутора часов полётов, а доля задержек
  ...
  отрезок нужно мерить в сутках, а не в строках (эксперимент 16).
...
- **Evidently 0.7**, `ValueDrift` на колонке ошибок: для бинарной колонки по умолчанию — Z-тест двух
  долей, дрейф при p < 0.05. Тест воспроизведён в коде (отчёт Evidently на каждое окно каждой модели — около 36 мс,
  десятки минут на прогон)
...
- **`split_common` снимает большую часть зависимости:** корреляция падает до 0.17–0.33, то есть
  до умеренной зоны ρ ≈ 0.15–0.35, где по эксперименту 5 потолок выигрыша совместного бутстрепа
  — 1.3–1.7 раза по уровню на модель, того же порядка, что и ошибка калибровки хвоста.
  Кучкование ложных тревог падает в 2.5–6 раз, на Electricity — лишь в 1.3 раза.
...
- На Electricity окон, где почти все модели в нулевом режиме, мало (в среднем 12 на повтор),
  поэтому её числа грубые.
...
режимов волатильности (затишье 2012–2014, 2015, 2020, 2022) ломают простейшие статические модели
по-настоящему: у среднего уровня и суточного профиля потеря гуляет по годам в пределах 0.76–1.03;
у HAR, регрессии на лагах и EWMA — 0.6–0.76.
...
### Эксперимент 23. Быстрее при том же бюджете ложных тревог

Три способа сократить задержку, все через `StreamingMonitor` шаг за шагом, с Бонферрони и одним
бюджетом ложных тревог: α = 0.05 на 100 шагов по всему парку.

- **Отбеливание** (`Prewhitened`): детектор работает на стандартизованных остатках AR-модели,
  оценённой на опорном отрезке.
- **Короткие окна:** 25 шагов вместо 100 при α/4, чтобы бюджет на 100 шагов был тем же.
- **Последовательный e-детектор** (`ECUSUM`): смешанный CUSUM e-детектор на отбелённых остатках,
  в оконном режиме и с проверкой на каждом шаге (`sequential=True`).

Синтетика: K = 100 моделей AR(1), φ = 0.5; резкий или постепенный дрейф на 0.5σ или 1σ у 10%
моделей; ρ = 0 или 0.6; 2 сида на случай, плюс сценарии без дрейфа по 6 сидов. Тревога ложная,
если среднее модели не менялось от начала её текущего опорного отрезка до тревоги. Реальные
данные — 40 моделей волатильности на курсах валют из эксперимента 22 (окно 20 торговых дней).

| метод | задержка, шагов | 1σ резкий | пропущено | F1 | ложная тревога на 100 шагов без дрейфа, ρ = 0 / 0.6 | мкс на модель за шаг |
|---|---|---|---|---|---|---|
| **e-CUSUM, каждый шаг** | **375** | **101–104** | 5% | 0.77 | 0.025 / 0.046 | 7.5 |
| e-CUSUM, окно 100 | 427 | 140–155 | 4% | 0.76 | 0.025 / 0.046 | 2.1 |
| MeanShift(3), окно 100 | 504 | 280–295 | 4% | 0.78 | 0.018 / 0.035 | 1.6 |
| PH, окно 100 | 506 | 210–230 | 11% | 0.75 | 0.011 / 0.025 | 1.8 |
| PH + отбеливание, окно 100 | 506 | 205–230 | 14% | 0.76 | 0.011 / 0.025 | 2.0 |
| PH, окно 25 | 521 | 197–229 | 23% | 0.73 | 0.007 / 0.011 | 5.4 |
| PH + отбеливание, окно 25 | 561 | 200–230 | 23% | 0.72 | 0.011 / 0.004 | 6.1 |

| курсы валют, δ = 0.05 | ложных тревог на модель в год | доля ложных | дней на испорченной модели в год |
|---|---|---|---|
| PH, окно 20 дней | 0 | 0 | 26.1 |
| MeanShift(3) | 0.011 | 0.22 | 28.4 |
| e-CUSUM, окно 20 дней | 0 | 0 | **21.2** |
| e-CUSUM, каждый шаг | 0.002 | 0.06 | **21.3** |

- **e-CUSUM — единственный из трёх способов, который реально ускоряет.** При том же бюджете ложных
  тревог последовательный e-CUSUM ловит дрейф на 26% быстрее PH и MeanShift(3), на резком сдвиге
  1σ — вдвое быстрее (около 100 шагов против 210–295). Это близко к грубой теоретической нижней
  границе для этого сценария (около 75 шагов); прежние детекторы были от неё в 2.5–4 раза.
  Уже оконный e-CUSUM даёт большую часть выигрыша (427 шагов): важнее то, *как* детектор копит
  свидетельство, чем то, как часто принимается решение.
- **На курсах валют e-CUSUM сокращает время на испорченной модели** с 26 до 21 дня на модель в год
  без ложных тревог в оконном режиме.
- **Отбеливание PH не помогло** (506 против 506 шагов, пропусков даже больше). Для сдвига среднего
  откалиброванный PH и так почти эффективен: частичные суммы AR-ряда несут ту же информацию, что
  суммы остатков. Отбеливание полезно как часть e-CUSUM, где оно даёт правильные e-значения, а не
  как отдельная обёртка.
- **Короткие окна хуже:** больше пропусков (23% против 11%) и задержка не меньше. Выигрыш от частых
  решений съедает более строгий уровень на каждую проверку (α/4), а шаг дороже втрое.
- **Последовательный режим нужно ограничивать горизонтом.** Первая версия копила CUSUM с момента
  переобучения бесконечно и превышала бюджет в 1.5–3 раза: небольшая ошибка оценки опорного среднего
  работает как слабый постоянный сдвиг, и бесконечная сумма его находит. Теперь поток держит по
  CUSUM на начало каждого из последних h окон и берёт самый старый — это ровно та статистика, под
  которую откалиброван порог. Цена — h обновлений на шаг: 7.5 мкс на модель вместо 2.
- **Пакетный расчёт статистик в конце окна** (одним векторизованным вызовом на группу одинаковых
  детекторов) ускоряет оконный шаг на 16–22%: в прямом сравнении на одной машине 1.97 против
  2.34 мкс на модель при 100 моделях и 2.13 против 2.72 при 1000.

Таблицы — `results/exp23_tables.md`. Валютная часть требует данных:
`python experiments/exp23_speed.py --fx КАТАЛОГ_С_CSV`; без флага она берётся из прошлого прогона.

### Эксперимент 24. Мониторинг без меток через CBPE

Когда метки приходят с задержкой, настоящая ошибка модели неизвестна. CBPE из NannyML оценивает её
по предсказанным вероятностям (`driftfdr.integrations.cbpe_estimated_error`). Проверено, можно ли
мониторить эту оценку вместо настоящей ошибки. Electricity, 50 логистических моделей, 2 повтора, в
двух вариантах: только естественные изменения и с внедрённым реальным дрейфом p(y|X) у 20% моделей
(метки переворачиваются с вероятностью 0.3). Шаг — 100 строк; опорный отрезок 40 шагов (с метками,
на нём обучается CBPE); окно 10, горизонт 4, δ = 5 п.п., Бонферрони. Тревога верна, если настоящая
ошибка за следующие 20 шагов выше опорной больше чем на δ.

| сигнал | корреляция с настоящей ошибкой | тревог (MeanShift(1)) | доля ложных | поймано ухудшений | поймано внедрённых дрейфов |
|---|---|---|---|---|---|
| настоящая ошибка, с метками | 1 | 10–12 | 0 | 13–14% | 18% |
| оценка CBPE, без меток | **−0.21…−0.25** | **0** | — | **0%** | **0%** |

- **На этих данных CBPE не заменяет метки.** Оценка ошибки не связана с настоящей (корреляция даже
  отрицательная) и систематически занижена: около 0.10–0.16 при настоящей 0.32–0.53 на проверенных
  моделях. У модели с внедрённым дрейфом настоящая ошибка выросла с 0.21 до 0.42, а оценка CBPE
  упала. CBPE опирается на откалиброванные вероятности и неизменную p(y|X); на нестационарном
  Electricity и то и другое нарушено, и мониторинг такой оценки молчит — без ложных тревог, но и без
  обнаружений.
- **Вывод для применения:** CBPE — сигнал для случая, когда меняется только распределение входов, а
  вероятности модели откалиброваны. Проверять это нужно на своих данных: сравнить оценку CBPE с
  настоящей ошибкой на отрезке, где метки уже пришли. Реальный дрейф p(y|X) без меток не виден
  (эксп. 11).
- **Сами данные трудные даже с метками:** ошибка на отрезке из 100 строк колеблется с отклонением
  около 0.19 и автокорреляцией 0.6, поэтому поймана лишь седьмая часть ухудшений при нулевой доле
  ложных тревог.
- **e-CUSUM не дал ни одной тревоги при опорном отрезке в 40 шагов.** Калибровка верная, но на 40
  точках приходится оценивать среднее и AR-модель, и бутстреп переносит эту неопределённость в широкое
  нулевое распределение. MeanShift(1) оценивает только среднее и на коротком отрезке мощнее.
  e-CUSUM нужен опорный отрезок хотя бы в несколько сотен точек (в эксп. 23 — 300).

Таблицы — `results/exp24_tables.md`. Запуск требует `pip install nannyml river`.

## Ограничения

- Большинство выводов получено на синтетике; реальных данных пять (четыре открытых набора и
  часовые курсы валют), размечен из них один (INSECTS, и то неполно), остальные оцениваются через
  оракул будущей ошибки. На всех реальных наборах модели парка предсказывают один таргет на одних
  и тех же данных, что завышает их корреляцию по сравнению с разнородным продовым парком.
...
- Решения принимаются только в конце окна. В потоковом мониторе у каждой модели свои часы, но
  в режиме `split_common` все модели должны присылать значение на каждом шаге; нерегулярные
  события сводятся к шагам через `bucket_means`.
- На шумном сигнале ловятся только существенные ухудшения: на курсах валют рост потери меньше
  ~15% неотличим от шума окна (эксп. 22).
...
из 100 сценариев (21), парк моделей волатильности на курсах валют (22), быстрее при том же
бюджете: отбеливание, короткие окна, последовательный e-CUSUM (23), мониторинг без меток через
CBPE (24).
...
| Онлайн-FDR: alpha-investing, LORD, SAFFRON и родственные | плюс LOND, BatchBH, BH, BH Стори, e-BH, Бонферрони в окне (эксп. 2–4, 12, 13); последовательный e-CUSUM с проверкой на каждом шаге (эксп. 23) | формальное сопоставление с ошибкой на терпение (EOP, Dandapanthula–Ramdas) — для статьи |
...
| Статья, валидация с практиком | черновик связанных работ (`docs/related_work.ru.md`, `docs/related_work.md`) | текст статьи; продовые логи с партнёром |

## План дальнейшего исследования

1. **Связанные работы для статьи:** черновик — [related_work.ru.md](related_work.ru.md); осталось
...
python experiments/exp23_speed.py --fx DATA_DIR  # ~40 мин; без --fx валютная часть из прошлого прогона
python experiments/exp24_cbpe.py           # ~10 мин; нужен pip install nannyml
```
