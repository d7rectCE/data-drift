...
то есть `ECUSUM`). Детектор работает с конца опорного отрезка, и на каждом шаге его текущий скор
сравнивается с нулевым распределением *наибольшего скора внутри окна* (тот же бутстреп, что в
оконном режиме). Горизонт просмотра ограничен так же, как в оконном режиме: поток держит по CUSUM
на начало каждого из последних `horizon` окон и берёт самый старый, так что статистика совпадает с
той, под которую откалиброван порог; неограниченный CUSUM медленно накапливал бы смещение от
оценки опорного среднего и превышал бюджет (эксп. 23). Тревога, как только p-значение опускается до α/K, держит
...
статистики считаются одним векторизованным вызовом на группу одинаковых детекторов (шаг дешевле на
16–22%, эксп. 23);
