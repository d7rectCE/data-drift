...
парки, где одновременно дрейфует меньше половины моделей; в потоковом монитор `split_common`
пока не встроен. Таблицы — `results/exp18_tables.md`.
...
1. **Остатки и тест парка в потоковом мониторе:** `split_common` пока работает только на
   готовых рядах (эксп. 18).
