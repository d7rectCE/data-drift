...
дрейфа (18), время работы (19), зависимость моделей на реальных данных (20).
...
1. **Точность, полнота и F1 обнаружения** в `summarize`, как в бенчмарке Cerqueira и др., чтобы
   результаты были сопоставимы с опубликованными.
2. **Единый бенчмарк из 100 сценариев** (эксп. 21): фиксированная сетка тип дрейфа × сила ×
   число потоков × корреляция, все детекторы на едином FAR, все основные правила.
3. **Связанные работы для статьи:** e-детекторы и EOP (Dandapanthula–Ramdas), «вредные» сдвиги
   (Podkopaev–Ramdas), CBPE в NannyML, Rebjock и др.; таблица соответствия постановок уже есть
   в обзоре литературы.
4. **Демонстрация в реальном времени** для защиты.
5. **Продовые логи** — нужен партнёр.
