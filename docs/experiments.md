...
1. **Единый бенчмарк из 100 сценариев** (эксп. 21): фиксированная сетка тип дрейфа × сила ×
   число потоков × корреляция, все детекторы на едином FAR, все основные правила.
2. **Связанные работы для статьи:** e-детекторы и EOP (Dandapanthula–Ramdas), «вредные» сдвиги
   (Podkopaev–Ramdas), CBPE в NannyML, Rebjock и др.; таблица соответствия постановок уже есть
   в обзоре литературы.
3. **Демонстрация в реальном времени** для защиты.
4. **Продовые логи** — нужен партнёр.
