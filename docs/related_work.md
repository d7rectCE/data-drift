...
показали эксперименты driftfdr (номера экспериментов — по [журналу](experiments.md)).
...
- **Что показали эксперименты.** При автокорреляции обычный бутстреп занижает частоту ложных
  тревог в разы, блочный и AR-sieve её выдерживают (эксп. 1). Хвост выдерживается для PH, KS и
  MeanShift, но не для ADWIN (эксп. 10). Детекторы на признаках ловят не тот дрейф: они
...
- **Что показали эксперименты.** Без поправки ложные тревоги растут линейно с числом моделей;
  ...
  ложные тревоги при корреляции придут пачками, подтвердилось: в одном окне их бывают
  десятки (эксп. 5, 18). На реальных наборах корреляция p-значений между моделями 0.4–0.9
